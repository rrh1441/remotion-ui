...
export const FadeOut: React.FC<FadeOutProps> = ({
  ...
  if (asChild) {
    if (!React.isValidElement(children)) {
      warnInDev.invalidChildren('FadeOut');
      return null;
    }

    if (React.Children.count(children) !== 1) {
      warnInDev.asChildMultipleChildren('FadeOut');
      return children as React.ReactElement;
    }

    const mergedStyle = { ...style, ...animationStyle };
    return mergeChildStyles(children, mergedStyle, 'FadeOut');
  }
  ...
  return (
    ...
    >
      {React.isValidElement(children) 
        ? mergeChildStyles(children, childStyle, 'FadeOut')
        : children
      }
    </div>
  );
