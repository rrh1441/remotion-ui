...
export const TTSProvider: React.FC<TTSProviderProps> = ({ config, children }) => {
  const synthesize = useMemo(() => {
    return async (text: string, options?: TTSOptions): Promise<TTSResult> => {
      const mergedOptions = {
        voice: options?.voice || config.defaultVoice || undefined,
...
async function synthesizeBrowser(text: string, options: Omit<Required<TTSOptions>, 'voice'> & { voice?: string | undefined }): Promise<TTSResult> {
...
async function synthesizeLocal(
  text: string,
  options: Omit<Required<TTSOptions>, 'voice'> & { voice?: string | undefined },
  config: TTSProviderConfig
): Promise<TTSResult> {
  if (!config.cachePath) {
